...
#define EXPORTING_compileLibCompiler 1
#include "compileLibCompiler.h"
    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("compileLibCompiler"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return compileLibCompilerInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "compile", nlhs, plhs, nrhs, prhs);
}

LIB_compileLibCompiler_C_API 
bool MW_CALL_CONV mlfCompile(int nargout, mxArray** output, mxArray* a)
{
    return mclMlfFeval(_mcr_inst, "compile", nargout, 1, 1, output, a);
}
